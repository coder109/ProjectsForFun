...
int main(int argc, char **argv) {
    ...
    std::vector<std::string> valid_complex_examples = {"2 3 + 4 *", "5 1 2 + 4 * + 3 -", "1.5 2.5 + 3.5 +"};
    std::vector<double> valid_complex_results = {20, 14, 7.5};
    std::vector<std::string> valid_edge_examples = {"    42    ", "    ", "", "   42   2    +", "1 3+"};
    std::vector<double> valid_edge_results = {42, 0, 0, 44, 4};
    std::vector<std::string> valid_decimal_examples = {"1.5 2.5 +", "1.5 2.5 *"};
    std::vector<double> valid_decimal_results = {4, 3.75};
    std::vector<std::string> valid_negative_examples = {"-1.5 2.5 +", "-1.5 2.5 *"};
    std::vector<double> valid_negative_results = {1, -3.75};
    std::vector<std::string> invalid_examples = {"2 3 + 4 * +", "5 1 2 + 4 * + - - 3 -", "3 0 /", "3 1 - 0 /", "   42    2   ", "-- + -- + --", "-  .  -  . . . -- . ", ".. - ", "."};
    std::vector<std::invalid_argument> invalid_results = {std::invalid_argument("Invalid expression"), std::invalid_argument("Invalid expression"), std::invalid_argument("Division by zero"), std::invalid_argument("Division by zero"), std::invalid_argument("Invalid expression"), std::invalid_argument("Invalid expression"), std::invalid_argument("Invalid expression"), std::invalid_argument("Invalid expression"), std::invalid_argument("Invalid expression")};
    ...
    std::cout << "Valid decimal examples" << std::endl;
    correct_num = 0;
    for(size_t i = 0; i < valid_decimal_examples.size(); ++i) {
        if(calc.Calculate(valid_decimal_examples[i]) == valid_decimal_results[i]) {
            std::cout << valid_decimal_examples[i] << " = " << valid_decimal_results[i] << std::endl;
            correct_num += 1;
        } else {
            std::cout << valid_decimal_examples[i] << " = " << calc.Calculate(valid_decimal_examples[i]) << " != " << valid_decimal_results[i] << std::endl;
        }
    }
    std::cout << "Correct num: " << correct_num << std::endl;
    std::cout << "Total num: " << valid_decimal_examples.size() << std::endl;

    std::cout << "Valid negative examples" << std::endl;
    correct_num = 0;
    for(size_t i = 0; i < valid_negative_examples.size(); ++i) {
        if(calc.Calculate(valid_negative_examples[i]) == valid_negative_results[i]) {
            std::cout << valid_negative_examples[i] << " = " << valid_negative_results[i] << std::endl;
            correct_num += 1;
        } else {
            std::cout << valid_negative_examples[i] << " = " << calc.Calculate(valid_negative_examples[i]) << " != " << valid_negative_results[i] << std::endl;
        }
    }
    std::cout << "Correct num: " << correct_num << std::endl;
    std::cout << "Total num: " << valid_negative_examples.size() << std::endl;
